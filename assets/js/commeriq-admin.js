        ...
        $(document).on('click', '#commeriq-activate-license', function (e) {
            ...
            $.post(commeriqAdmin.ajax_url, postData).done(function (resp) {
                ...
                if (xhr.responseJSON && xhr.responseJSON.data) {
                    ...
                    if (xhr.responseJSON.data.debug_info) {
                        var debug = xhr.responseJSON.data.debug_info;
                        errorMsg += ' [Code: ' + debug.response_code + ']';
                        if (debug.response_body) {
                            errorMsg += ' Body: ' + debug.response_body.substring(0, 100);
                        }
                    }
                }

                $('#commeriq-modal-message').text(errorMsg);
                $('#commeriq-modal-close').show();
                $btn.data('processing', false).prop('disabled', false).text(originalText);
            });
        });
