...
class ReportsPage
{
    ...
    public static function handle_export_csv()
    {
        ...
        if (empty($ids)) {
            wp_safe_redirect(admin_url('admin.php?page=commeriq-reports&error=no_selection'));
            exit;
        }
        ...
        // Build query with escaped table name and prepare with IDs
        $query = "SELECT * FROM `" . esc_sql($table) . "` WHERE id IN ({$placeholders}) ORDER BY created_at DESC";
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Table name escaped, dynamic placeholders validated, direct query necessary for CSV export
